...
SELECTORS = {
    # VERIFY against current ysense signup page
    "first_name": "input[name=first_name]",
    "last_name": "input[name=last_name]",
    "email": "input[name=email], input[type=email]",
    "password": "input[name=password], input[type=password]",
    "country": "select[name=country], #country",
    "dob_year": "select[name=year], #year",
...
@register
class YSenseAdapter(SignupAdapter):
    target_id = "ysense"
    landing_url = "https://www.ysense.com/?rb="
    ...
    async def run(self, page: Page, ctx: SignupContext) -> SignupResult:
        ...
        try:
            await fill_human(page, SELECTORS["first_name"], ctx.identity.first_name)
            await fill_human(page, SELECTORS["last_name"], ctx.identity.last_name)
            await fill_human(page, SELECTORS["email"], ctx.inbox.address)
            await fill_human(page, SELECTORS["password"], ctx.identity.password)
            try:
                await page.select_option(SELECTORS["country"], ctx.identity.country)
            except Exception:
                pass
            try:
                await page.select_option(SELECTORS["dob_year"], str(ctx.identity.dob.year))
            except Exception:
                pass
        ...
        if ctx.target.requires_email_verify:
            link = await ctx.mail.wait_for_link(ctx.inbox, contains="ysense")
            if not link:
                return SignupResult(ok=False, note="no ysense verification email")
