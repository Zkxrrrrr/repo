...
SELECTORS = {
    # VERIFY against current freecash signup page
    "email": "input[name=email], input[type=email]",
    "password": "input[name=password], input[type=password]",
    "username": "input[name=username]",
    "submit": "button[type=submit], button:has-text('Sign Up')",
    "cookie_accept": "button:has-text('Accept'), button:has-text('Allow all')",
}


@register
class FreecashAdapter(SignupAdapter):
    target_id = "freecash"
    landing_url = "https://freecash.com/register"
    ...
    async def run(self, page: Page, ctx: SignupContext) -> SignupResult:
        ...
        try:
            try:
                ...
                pass  # some variants only require email
        ...
        if ctx.target.requires_email_verify:
            link = await ctx.mail.wait_for_link(ctx.inbox, contains="freecash")
            if not link:
                return SignupResult(ok=False, note="no freecash verification email")
