...
SELECTORS = {
    # VERIFY against current inboxdollars signup page
...
@register
class InboxDollarsAdapter(SignupAdapter):
    target_id = "inboxdollars"
    landing_url = "https://www.inboxdollars.com/registration"

    async def run(self, page: Page, ctx: SignupContext) -> SignupResult:
        url = self.referral_url_with_subid(
            self.landing_url, ctx.target.referral_code, ctx.signup_id, ref_param="ref"
        )
        ...
        try:
            ...
            try:
                await fill_human(page, SELECTORS["zip"], ctx.identity.zip_code)
            except Exception:
                pass  # zip field may be optional/absent on certain variants
        ...
        if ctx.target.requires_email_verify:
            link = await ctx.mail.wait_for_link(ctx.inbox, contains="inboxdollars")
            if not link:
                return SignupResult(ok=False, note="no inboxdollars verification email")
