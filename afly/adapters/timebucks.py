...
SELECTORS = {
    # VERIFY against current timebucks signup page
...
@register
class TimeBucksAdapter(SignupAdapter):
    target_id = "timebucks"
    landing_url = "https://timebucks.com/?refid="

    async def run(self, page: Page, ctx: SignupContext) -> SignupResult:
        # timebucks uses ?refid=CODE directly in URL; subid added separately
        ...
        if ctx.target.requires_email_verify:
            link = await ctx.mail.wait_for_link(ctx.inbox, contains="timebucks")
            if not link:
                return SignupResult(ok=False, note="no timebucks verification email")
